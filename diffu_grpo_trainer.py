...
class DiffuGRPOTrainer(GRPOTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        trajectory_step_map = inputs["trajectory_step_map"]          # [B, comp_len]
        trajectory_step_indices = inputs["trajectory_step_indices"]  # [num_iterations, B]

        input_ids = torch.cat([prompt_ids, completion_ids], dim=1)
        logits_to_keep = completion_ids.size(1)

        this_itr_idx = self._step % self.args.num_iterations
        this_itr_trajectory_step_idx = trajectory_step_indices[this_itr_idx]  # [B]

        delta_mask = (trajectory_step_map == this_itr_trajectory_step_idx.unsqueeze(-1)).int()  # [B, comp_len]
        active_mask = delta_mask * completion_mask
        input_ids = input_ids.unsqueeze(0)
        per_token_logps = self._get_per_token_logps_from_trajectory(
            model, input_ids, logits_to_keep,
            trajectory_step_map, this_itr_trajectory_step_idx.unsqueeze(0)
        ).squeeze(0)  # [B, comp_len]
        ...
        if self.beta != 0.0:
            mean_kl = (per_token_kl * active_mask).sum() / active_mask.sum()
            self._metrics[mode]["kl"].append(self.accelerator.gather_for_metrics(mean_kl).mean().item())

        is_clipped = (per_token_loss1 < per_token_loss2).float()
        clip_ratio = (is_clipped * active_mask).sum() / active_mask.sum()
    ...
    def _get_per_token_logps_from_trajectory(self, model, input_ids, logits_to_keep, trajectory_step_map, trajectory_step_indices):
        """
        Calculate per-token log probabilities using trajectory-based masking.

        Structurally mirrors _get_per_token_logps, but instead of applying random masks
        via forward_process, it reconstructs partially-masked inputs from the trajectory
        step map for each iteration.
        """
        num_iterations, batch_size, seq_len = input_ids.size()
        device = input_ids.device

        assert (
            trajectory_step_indices.size(0) == num_iterations
        ), f"Expected trajectory_step_indices dim 0 to be {num_iterations}, got {trajectory_step_indices.size(0)}"

        prompt_length = seq_len - logits_to_keep
        prompt_index = torch.zeros(seq_len, dtype=torch.bool, device=device)
        prompt_index[:prompt_length] = True

        # applying masks from trajectory step map
        all_perturbed_seqs = []
        all_expanded_inputs = []
        for iter_idx in range(num_iterations):
            expanded_input = input_ids[iter_idx]  # [batch_size, seq_len]
            step_idx = trajectory_step_indices[iter_idx]  # [batch_size]
            visible = trajectory_step_map < step_idx.unsqueeze(-1)  # [batch_size, comp_len]
            completion_ids = expanded_input[:, prompt_length:]
            completion_input = torch.where(visible, completion_ids, self.args.mask_id)
            perturbed_seq = torch.cat([expanded_input[:, :prompt_length], completion_input], dim=1)
    ...
    def _generate_and_score_completions(
        ...
        '''
            Feasible path rollout via FreeDave (https://arxiv.org/abs/2510.00294)
        '''
        ...
        with unwrap_model_for_generation(self.model_wrapped, self.accelerator) as unwrapped_model:
            ...
            trajectory_step_maps_all = []

            for i in range(prompt_ids.size(0)):
                batch_prompt_ids = prompt_ids[i : i + 1] # use sub_batch_size=1 here
                ...
                batch_prompt_completion_ids, batch_trajectory_step_map = self.generate_with_trajectory(
            ...
            prompt_completion_ids = torch.cat(prompt_completion_ids_all, dim=0)  # [batch_size, seq_len]
            trajectory_step_map = torch.cat(trajectory_step_maps_all, dim=0)     # [batch_size, comp_len]
        ...
        return {
            ...
            "trajectory_step_map": trajectory_step_map,          # [B, comp_len]
            "trajectory_step_indices": trajectory_step_indices,  # [num_iterations, B]
